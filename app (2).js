...
function ordersSummaryHtml(c){
  ...
  const byStore = {};
  orders.forEach(o=>{ const r = o.restaurant||'Unknown'; byStore[r] = (byStore[r]||0) + (parseFloat(o.amount)||0); });
  ...
  return `
    <div class="card" style="background:var(--paper);">
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;">
        <div class="stat-chip"><span class="n">${orders.length}</span><span class="l">Orders</span></div>
      ...
      <div>
        ${sorted.slice(0,25).map(o=>`
          <div class="order-row">
            <div>
              <div>${escapeHtml(o.restaurant||'')}${o.orderId?' · #'+escapeHtml(o.orderId):''}${o.promotion?' · <span style="color:var(--gold);">'+escapeHtml(o.promotion)+'</span>':''}</div>
              ${o.comment?`<div class="l">${escapeHtml(o.comment)}</div>`:''}
            </div>
            <div style="text-align:right;white-space:nowrap;">
              <div>$${(parseFloat(o.amount)||0).toFixed(2)}</div>
...
const IMPORT_FIELDS = [
  ...
  {key:'comment', label:'Comment / notes'}
];
...
function guessImportField(header){
  ...
  if(/comment|note|remark/.test(h)) return 'comment';
...
document.getElementById('runImportBtn').onclick = async ()=>{
  ...
  try{
    ...
    const updates = new Map();       // existing customer.id -> { customer, addOrders, seenKeys }
    const newCustomers = new Map();  // batch key -> { name, phone, email, orders, seenKeys }
    let skippedNoName = 0, duplicateOrders = 0, totalOrdersQueued = 0;
    ...
    importRows.forEach(row=>{
      ...
      const order = { orderId, date, restaurant, amount, promotion, comment, importedAt: new Date().toISOString() };
      ...
      if(existing){
        if(!updates.has(existing.id)){
          const seenKeys = new Set((existing.orders||[]).map(o=> orderKey(o, existing.name)));
          updates.set(existing.id, { customer: existing, addOrders: [], seenKeys });
        }
        const bucket = updates.get(existing.id);
        const dk = orderKey(order, existing.name);
        if(bucket.seenKeys.has(dk)){ duplicateOrders++; return; }
        bucket.seenKeys.add(dk); bucket.addOrders.push(order); totalOrdersQueued++;
      }else{
        const batchKey = key || normalize(email) || normalize(name);
        if(!newCustomers.has(batchKey)) newCustomers.set(batchKey, { name, phone, email, orders: [], seenKeys: new Set() });
        const nb = newCustomers.get(batchKey);
        const dk = orderKey(order, name);
        if(nb.seenKeys.has(dk)){ duplicateOrders++; return; }
        nb.seenKeys.add(dk); nb.orders.push(order); totalOrdersQueued++;
    ...
    const ops = [];
    updates.forEach(({customer, addOrders})=>{
      if(!addOrders.length) return;
      ops.push({ type:'update', ref: db.collection('customers').doc(customer.id), data: { orders: [...(customer.orders||[]), ...addOrders] } });
    });
    newCustomers.forEach(nb=>{
      ops.push({ type:'set', ref: db.collection('customers').doc(), data: {
        ...
        incidents:[], orders: nb.orders,
        auditLog:[{ date:new Date().toISOString(), action:`Created via Excel import (${nb.orders.length} order${nb.orders.length>1?'s':''})` }]
      }});
    });
