...
const CATEGORY_ORDER = ['Vegetables & Produce','Meat & Poultry','Dairy','Frozen','Drinks & Beverages','Indian Store Grocery','Kitchen Essentials','Packing Material','Cleaning & Sanitation','Snacks','Bakery','General / Canned Goods','Other'];
...
function startListeners(){
  ...
  db.collection('vaultCodes').onSnapshot(snap=>{
    vaultCodes = [];
    snap.forEach(doc=> vaultCodes.push({ id: doc.id, ...doc.data() }));
    console.log('[listeners] vaultCodes synced:', vaultCodes.length, 'records');
    vaultRenderDashboard();
  }, err=>{
    console.error('[listeners] vaultCodes listener FAILED:', err);
    toast('Could not load rewards data: '+err.message, 'error');
  });
  db.collection('vaultConfig').doc('settings').onSnapshot(doc=>{
    vaultConfig = doc.exists ? doc.data() : { tiers: DEFAULT_VAULT_TIERS, streakN: 5 };
    document.getElementById('v-streakN').value = vaultConfig.streakN || 5;
    vaultRenderTierRows();
  }, err=>{
    console.error('[listeners] vaultConfig listener FAILED:', err);
    toast('Could not load reward tier settings: '+err.message, 'error');
  });
...
function restTagHtml(name){
  if(!name) return '';
  let hash = 0;
  for(let i=0;i<name.length;i++) hash = (hash*31 + name.charCodeAt(i)) >>> 0;
  const colorIdx = hash % 5;
  return `<span class="rest-tag rest-${colorIdx}">${escapeHtml(name)}</span>`;
}
...
function safetyLevel(c){
  ...
  if(c.statusTag==='review' || c.statusTag==='onetime') return {level:'yellow', label:'Doubt'};
  if(c.statusTag==='trusted' || c.statusTag==='repeat') return {level:'green', label:'Safe'};
...
function custRowHtml(c){
  ...
  return `<div class="cust-row" data-cust-id="${c.id}">
    <div>
      <div style="margin-bottom:4px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">${safetyBadgeHtml(c)}${restTagHtml(rest)}</div>
      <div class="name ${colorClass}">${escapeHtml(c.name)}${tag}</div>
...
function orderStatusBadge(o){
  if(o.isCancelledFlag) return `<span class="stamp stamp-red" style="margin-left:6px;">Cancelled</span>`;
  if(o.status){
  ...
  if(o.isMissingFlag || o.errorCategory) return `<span class="stamp stamp-gold" style="margin-left:6px;">${escapeHtml(o.errorCategory||'Missing/incorrect')}</span>`;
  if(o.customerEmoji==='disliked') return `<span class="stamp stamp-gold" style="margin-left:6px;">Disliked</span>`;
  if(o.customerEmoji==='loved'||o.customerEmoji==='liked') return `<span class="stamp stamp-green" style="margin-left:6px;">${o.customerEmoji==='loved'?'❤️ Loved':'👍 Liked'}</span>`;
...
function ordersSummaryHtml(c){
  const orders = c.orders || [];
  if(!orders.length) return '';
  const isBadOrder = o=> !!o.isCancelledFlag || !!o.isMissingFlag
    || (o.status && /cancel|error|charge.?back|dispute|fraud|declin|fail|reject/i.test(o.status))
    || !!o.comment || !!o.errorCategory || !!(parseFloat(o.errorCharge)||0) || o.customerEmoji==='disliked';
    ...
    <div class="card" style="background:var(--paper);">
      ...
      <div>
        ${sorted.slice(0,25).map(o=>{
          ...
          <div class="order-row">
            <div>
              <div>${restTagHtml(o.restaurant||'Unknown restaurant')}${o.orderId?' #'+escapeHtml(o.orderId):''}${orderStatusBadge(o)}${o.promotion?' · <span style="color:var(--gold);">'+escapeHtml(o.promotion)+'</span>':''}</div>
...
function showCustomerDetail(id){
  ...
  // Which imported files contributed data to THIS customer, so a bad import can be
  // undone for just this one profile without affecting anyone else it also touched.
  const batchIdsHere = new Set();
  (c.orders||[]).forEach(o=> { if(o.importBatchId) batchIdsHere.add(o.importBatchId); });
  (c.incidents||[]).forEach(inc=> { if(inc.importBatchId) batchIdsHere.add(inc.importBatchId); });
  const batchesHere = Array.from(batchIdsHere).map(id=> importBatchesCache.find(b=>b.id===id)).filter(Boolean);
  const importedDataHtml = batchesHere.length ? `
    <div class="card" style="background:var(--paper);margin-top:10px;">
      <div style="font-size:13px;font-weight:600;margin-bottom:8px;">Imported data on this profile</div>
      ${batchesHere.map(b=>`
        <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--line);font-size:12.5px;">
          <span>${escapeHtml(b.fileName||'Unnamed file')} · ${fmtDate(b.importedAt)}</span>
          <button class="btn-danger" data-remove-batch-here="${b.id}" style="font-size:11.5px;padding:5px 9px;">Remove this file's data</button>
        </div>`).join('')}
    </div>` : '';
  ...
  document.getElementById('custDetail').innerHTML = `
    <div class="card">
      ${stamp}
      <div style="margin:10px 0 4px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">${safetyBadgeHtml(c)}${restTagHtml(primaryRestaurant(c))}</div>
      <div style="font-size:19px;font-weight:600;margin:4px 0 2px;" class="${nameColorClass(incidents.length)}">${escapeHtml(c.name)}</div>
  ...
  `;
  document.querySelectorAll('[data-remove-batch-here]').forEach(btn=>{
    btn.onclick = async ()=>{
      if(!confirm('Remove this imported file\'s orders/incidents from just this customer?')) return;
      const batchId = btn.getAttribute('data-remove-batch-here');
      showLoading('Removing…');
      try{
        const orders = (c.orders||[]).filter(o=> o.importBatchId !== batchId);
        const incidentsLeft = (c.incidents||[]).filter(i=> i.importBatchId !== batchId);
        const al = c.auditLog || [];
        al.push({ date:new Date().toISOString(), action:'Removed data from one imported file' });
        await withTimeout(db.collection('customers').doc(c.id).update({ orders, incidents: incidentsLeft, auditLog: al }), 'Remove batch from customer');
...
const IMPORT_FIELDS = [
  ...
  {key:'errorCategory', label:'Error category / cancellation category'},
  {key:'status', label:'Order status (completed / cancelled / error / refund…)'},
  {key:'isCancelledFlag', label:'"Is cancelled" (true/false column)'},
  {key:'isMissingFlag', label:'"Is missing or incorrect" (true/false column)'},
  {key:'itemName', label:'Item name'},
  {key:'dasherName', label:'Dasher name'},
  {key:'cancelledBy', label:'Cancelled or handled by / non-payment reason'},
  {key:'promotion', label:'Promotion / discount'},
  {key:'comment', label:'Customer comment / notes / reason'},
  {key:'customerType', label:'Customer type (new/repeat/unknown)'},
  {key:'customerRating', label:'Customer rating (stars)'},
  {key:'customerEmoji', label:'Customer emoji rating'}
];

// DoorDash's own exports use the literal string "NULL" for blank cells — treat it as empty
// everywhere, or it pollutes comments/reasons/names with the text "NULL".
function cleanVal(v){
  if(v===null || v===undefined) return '';
  const s = String(v).trim();
  return (s.toUpperCase()==='NULL' || s==='NaN') ? '' : s;
}
...
// Detects which of DoorDash's standard exports a file is, from its header row, so the
// importer can apply the right rule (e.g. every row in the "cancelled orders" export IS
// a cancellation, even though no single column says so in words).
function detectFileType(headers){
  const set = headers.map(h=>(h||'').toString().toLowerCase());
  const has = re => set.some(h=>re.test(h));
  if(has(/cancellation category/) || (has(/cancelled at timestamp/) && has(/customer name/))) return 'cancelled';
  if(has(/error category/) && has(/customer comment/)) return 'missing_incorrect';
  if(has(/avoidable wait time/)) return 'avoidable_wait';
  if(has(/^is cancelled$/) && has(/^is missing or incorrect$/) && !has(/customer name/)) return 'ledger';
  return 'custom';
}
const FILE_TYPE_LABELS = {
  ledger: 'DoorDash "Sales by Order" — full order ledger (no customer names; enriches orders already known from other files)',
  avoidable_wait: 'DoorDash "Avoidable Wait Orders" — operational timing data, not a defaulter signal',
  cancelled: 'DoorDash "Cancelled Orders" — every row here is treated as a cancellation incident',
  missing_incorrect: 'DoorDash "Missing / Incorrect Orders" — every row here is treated as an error/comment incident',
  custom: 'Other / custom spreadsheet — uses status, comment, and error-charge columns to decide incidents'
};

function guessImportField(header, fileType){
  const h = (header||'').toString().toLowerCase();
  if(/dd.?order|order.?(id|no|number|#)/.test(h)) return 'orderId';
  if(/dasher/.test(h)) return 'dasherName';
  if(/^is cancelled$/.test(h)) return 'isCancelledFlag';
  if(/^is missing or incorrect$/.test(h)) return 'isMissingFlag';
  if(/cancellation category/.test(h)) return 'errorCategory';
  if(/non-?payment reason/.test(h)) return 'cancelledBy';
  if(/error.?(category|type|reason)/.test(h)) return 'errorCategory';
  if(/error.?charge|charge.?amount/.test(h)) return 'errorCharge';
  if(/customer type/.test(h)) return 'customerType';
  if(/customer rating/.test(h)) return 'customerRating';
  if(/customer emoji/.test(h)) return 'customerEmoji';
  ...
  if(/customer name/.test(h)) return 'name';
  if(/name/.test(h) && !/store|restaurant|item|product|dasher|business/.test(h)) return 'name';
  if(/phone|mobile|contact/.test(h)) return 'phone';
  if(/e[-\s]?mail/.test(h)) return 'email';
  if(/store name|restaurant|business name|location|branch|outlet/.test(h)) return 'restaurant';
  if(/deliver(y|ed).?date/.test(h)) return 'deliveryDate';
  ...
  if(/subtotal|amount|total|spent|price|paid|payout/.test(h)) return 'amount';
  return 'ignore';
}

// Decides whether a row represents a defaulter-worthy incident, and why. This is the
// piece that was previously the same for every spreadsheet regardless of what it actually
// was — now it branches by DoorDash file type, since "cancelled orders" and "missing/
// incorrect orders" exports are ALREADY pre-filtered to problem orders (every row counts),
// while the full order ledger only counts if its own true/false flags say so.
function classifyRow(fileType, row){
  const { statusText, comment, errorCategory, errorCharge, isCancelledFlag, isMissingFlag } = row;
  if(fileType === 'cancelled'){
    return { issue: errorCategory ? `Cancelled — ${errorCategory}` : 'Order cancelled' };
  }
  if(fileType === 'missing_incorrect'){
    return { issue: errorCategory ? `${errorCategory}` : 'Missing / incorrect order' };
  }
  if(fileType === 'avoidable_wait'){
    return null; // operational timing data only — never a customer-fault signal
  }
  if(fileType === 'ledger'){
    if(isCancelledFlag) return { issue:'Order cancelled' };
    if(isMissingFlag) return { issue:'Missing / incorrect order' };
    if(errorCharge) return { issue:'Error charge' };
    return null;
  }
  // custom/other spreadsheet — fall back to keyword + comment/error-charge based detection
  const kw = classifyOrderStatus(statusText, (statusText||errorCategory) ? '' : comment, errorCategory);
  if(kw) return { issue: kw.issue };
  if(comment || errorCharge) return { issue: errorCharge ? 'Error charge' : 'Customer comment on order' };
  return null;
}
...
function mergeOrderIntoList(orders, incoming, custName){
  ...
  if(idx === -1) return { orders: [...orders, incoming], isNew:true, isMerge:false, mergedOrder: incoming };
  const existing = orders[idx];
  const merged = { ...existing };
  ['restaurant','date','deliveryDate','amount','errorCharge','promotion','comment','status','errorCategory','dasherName','orderId','customerType','customerRating','customerEmoji'].forEach(f=>{
    if((merged[f]===undefined || merged[f]==='' || merged[f]===0 || merged[f]===null) && incoming[f]) merged[f] = incoming[f];
  });
  if(incoming.isCancelledFlag) merged.isCancelledFlag = true;
  if(incoming.isMissingFlag) merged.isMissingFlag = true;
  ...
  return { orders: next, isNew:false, isMerge: changed, mergedOrder: merged };
}
...
function resetImportModal(){
  importRows = []; importHeaders = []; importMapping = {}; detectedFileType = 'custom';
...
document.getElementById('importFile').onchange = (e)=>{
  ...
  reader.onload = (ev)=>{
    try{
      ...
      detectedFileType = detectFileType(importHeaders);
...
function renderImportMapping(){
  const typeWrap = document.getElementById('importDetectedType');
  if(typeWrap){
    typeWrap.innerHTML = `
      <div class="card" style="background:var(--gold-bg);border-color:#e8d3ab;margin-bottom:14px;">
        <div style="font-size:12.5px;font-weight:700;text-transform:uppercase;letter-spacing:.3px;color:var(--text-dim);margin-bottom:4px;">Detected file type</div>
        <select id="importTypeOverride" style="margin-bottom:6px;">
          ${Object.keys(FILE_TYPE_LABELS).map(k=>`<option value="${k}" ${k===detectedFileType?'selected':''}>${FILE_TYPE_LABELS[k]}</option>`).join('')}
        </select>
        <div style="font-size:12px;color:var(--text-dim);">Wrong? Pick the correct type above — it changes which rows count as a defaulter incident.</div>
      </div>`;
    document.getElementById('importTypeOverride').onchange = (e)=>{
      detectedFileType = e.target.value;
      // Re-guess every column against the newly selected type, since the right column to
      // treat as "the incident reason" differs between, say, cancelled-orders and ledger files.
      importHeaders.forEach((h,i)=> importMapping[i] = guessImportField(h, detectedFileType));
      renderImportMapping();
    };
  }
  ...
  wrap.innerHTML = importHeaders.map((h,i)=>{
    const guess = importMapping[i] !== undefined ? importMapping[i] : guessImportField(h, detectedFileType);
  ...
  wrap.querySelectorAll('[data-map-col]').forEach(sel=>{
    sel.onchange = ()=>{ importMapping[parseInt(sel.dataset.mapCol)] = sel.value; renderImportPreviewTable(); };
  });
  const typeNote = detectedFileType==='cancelled' ? ' Every row in this file will be logged as a cancellation incident, regardless of what the mapped columns say.'
    : detectedFileType==='missing_incorrect' ? ' Every row in this file will be logged as an error/incorrect-order incident, regardless of what the mapped columns say.'
    : detectedFileType==='avoidable_wait' ? ' This file never creates a defaulter incident — it only adds timing info to matching orders.'
    : detectedFileType==='ledger' ? ' This file has no customer names — it only enriches orders that already exist from another import, matched by DD Order ID.'
    : '';
  document.getElementById('importPreviewInfo').textContent = `${importRows.length} order rows ready to import. Rows with no value in the "Customer name" column will be skipped.${typeNote}`;
  renderImportPreviewTable();
}

// Shows exactly what will be extracted from the first few rows, using the CURRENT
// mapping — this is the fastest way to catch a wrong guess (e.g. "Restaurant" or
// "Comment" mapped to "Ignore") before committing the import.
function renderImportPreviewTable(){
  ...
  const previewFields = ['name','phone','restaurant','orderId','date','amount','status','errorCategory','errorCharge','comment','dasherName'];
  const sampleRows = importRows.slice(0,3);
  const el = document.getElementById('importPreviewTable');
  if(!el) return;
  if(!sampleRows.length){ el.innerHTML=''; return; }
  el.innerHTML = `
    <div style="font-size:12.5px;font-weight:600;margin:14px 0 6px;">Preview — what will actually be imported (first ${sampleRows.length} rows):</div>
    <div style="overflow-x:auto;">
    <table style="border-collapse:collapse;width:100%;font-size:12px;">
      <thead><tr>${previewFields.map(f=>`<th style="text-align:left;padding:5px 8px;border-bottom:2px solid var(--line);color:var(--text-dim);text-transform:uppercase;font-size:10.5px;">${IMPORT_FIELDS.find(x=>x.key===f).label}</th>`).join('')}</tr></thead>
      <tbody>
        ${sampleRows.map(row=>{
          const get = (field)=> colByField[field]!==undefined ? cleanVal(row[colByField[field]]) : '';
          return `<tr>${previewFields.map(f=>{
            const v = get(f);
            const isBlank = v===''||v===undefined||v===null;
            return `<td style="padding:5px 8px;border-bottom:1px solid var(--paper);${isBlank?'color:var(--text-dim);font-style:italic;':''}">${isBlank?'(blank)':escapeHtml(String(v))}</td>`;
          }).join('')}</tr>`;
        }).join('')}
      </tbody>
    </table>
    </div>
    <div style="font-size:11.5px;color:var(--text-dim);margin-top:6px;">If a column you expect to see data in shows "(blank)" for all 3 rows, that column is mapped wrong — fix it in the dropdowns above before importing.</div>`;
}

document.getElementById('runImportBtn').onclick = async ()=>{
  const colByField = {};
  Object.entries(importMapping).forEach(([idx,field])=>{ if(field!=='ignore' && !(field in colByField)) colByField[field] = parseInt(idx); });
  const isLedger = detectedFileType === 'ledger';
  if(colByField.name === undefined && !isLedger){ toast('Map at least one column to "Customer name" before importing', 'error'); return; }

  if(colByField.restaurant === undefined){
    const proceed = confirm('No column is mapped to "Restaurant / store" — every imported order will use the default restaurant you picked instead of anything from your file. Continue anyway?');
    if(!proceed) return;
  }
  if(!isLedger && detectedFileType==='custom' && colByField.status === undefined && colByField.errorCategory === undefined && colByField.comment === undefined){
    const proceed = confirm('No column is mapped to "Order status," "Error category," or "Comment" — nothing in this file will be able to trigger a defaulter flag. Continue anyway?');
    if(!proceed) return;
  }
  ...
  const fileNameForBatch = (document.getElementById('importFileName').textContent||'').split(' — ')[0] || 'Unnamed file';
  ...
  try{
    ...
    // Cross-customer order-ID index, so a "Sales by Order" ledger row (no name at all) can
    // still find and enrich the exact order created earlier from the cancelled/missing/
    // avoidable-wait files, purely by matching DD Order ID.
    const custByOrderId = new Map();
    customers.forEach(c=> (c.orders||[]).forEach(o=> { if(o.orderId) custByOrderId.set(normalize(o.orderId), c); }));
    ...
    let skippedNoName = 0, newOrders = 0, mergedOrders = 0, incidentsFlagged = 0, mergedIncidents = 0, orphanLedgerRows = 0;

    function getBucketForExisting(existing){
      ...
      return updates.get(existing.id);
    }
    ...
    importRows.forEach(row=>{
      ...
      const restaurant = get('restaurant') || defaultRestaurant;
      const orderId = get('orderId');
      const date = excelDateToISO(get('date'));
      const deliveryDate = excelDateToISO(get('deliveryDate'));
      const amountRaw = get('amount').replace(/[^0-9.-]/g,'');
      const amount = amountRaw ? parseFloat(amountRaw) : 0;
      const errorChargeRaw = get('errorCharge').replace(/[^0-9.-]/g,'');
      ...
      const isCancelledFlag = colByField.isCancelledFlag!==undefined ? parseBoolCell(row[colByField.isCancelledFlag]) : false;
      const isMissingFlag = colByField.isMissingFlag!==undefined ? parseBoolCell(row[colByField.isMissingFlag]) : false;
      const itemName = get('itemName');
      const dasherName = get('dasherName');
      const cancelledBy = get('cancelledBy');
      const promotion = get('promotion');
      const comment = get('comment');
      const customerType = get('customerType');       // new / repeat / unknown
      const customerRating = get('customerRating');
      const customerEmoji = get('customerEmoji');

      if(!name && !orderId){ skippedNoName++; return; }

      const order = {
        orderId, date, deliveryDate, restaurant, amount, errorCharge, errorCategory,
        promotion, comment, status: statusText, dasherName, customerType, customerRating, customerEmoji,
        isCancelledFlag, isMissingFlag,
        items: itemName ? [itemName] : [], importedAt: new Date().toISOString(), importBatchId: batchId
      };

      const classification = classifyRow(detectedFileType, { statusText, comment, errorCategory, errorCharge, isCancelledFlag, isMissingFlag });
      let incident = null;
      if(classification){
        const noteParts = [];
        if(comment) noteParts.push(comment);
        if(dasherName) noteParts.push(`Dasher: ${dasherName}`);
        if(cancelledBy) noteParts.push(`Cancelled/handled by / reason: ${cancelledBy}`);
        incident = {
          ...
          shots: [], importBatchId: batchId
        };
      }

      // No name on this row (typically the "Sales by Order" ledger export) — it can only
      // enrich an order that already exists somewhere, matched purely by DD Order ID.
      if(!name){
        const existing = orderId ? custByOrderId.get(normalize(orderId)) : null;
        if(!existing){ orphanLedgerRows++; return; }
        const bucket = getBucketForExisting(existing);
        const or = mergeOrderIntoList(bucket.orders, order, bucket.name);
        bucket.orders = or.orders;
        if(or.isMerge) { mergedOrders++; bucket.ordersChanged = true; }
        if(incident){
      ...
      let existing = (phone ? custByKey.get(key) : null) || (email ? custByEmail.get(normalize(email)) : null)
                   || (orderId ? custByOrderId.get(normalize(orderId)) : null) || custByName.get(normalize(name));
      ...
      if(existing){
        ...
        // Also index this brand-new customer by order ID immediately, so a LATER row in
        // this SAME file (e.g. a second item line for the same order) merges correctly
        // even before the batch is committed to Firestore.
        if(orderId) custByOrderId.set(normalize(orderId), { id:'__pending__', name, orders: nb.orders, incidents: nb.incidents });
      }
    });
    ...
    updates.forEach(({customer, orders, incidents, ordersChanged, incidentsChanged})=>{
      ...
      if(incidentsChanged){
        ...
        auditLog.push({ date:new Date().toISOString(), action:`Order/incident data updated from Excel import (${fileNameForBatch})` });
    ...
    newCustomers.forEach(nb=>{
      ops.push({ type:'set', ref: db.collection('customers').doc(), data: {
        ...
        auditLog:[{ date:new Date().toISOString(), action:`Created via Excel import — ${fileNameForBatch} (${nb.orders.length} order${nb.orders.length>1?'s':''}${nb.incidents.length?`, ${nb.incidents.length} flagged incident${nb.incidents.length>1?'s':''}`:''})` }]
      }});
    });
    ...
    // Record this import so it can be found and un-done later from "Import history".
    await withTimeout(db.collection('importBatches').doc(batchId).set({
      fileName: fileNameForBatch, fileType: detectedFileType, importedAt: new Date().toISOString(),
      rowCount: importRows.length, customersUpdated: updates.size, customersCreated: newCustomers.size,
      newOrders, mergedOrders, incidentsFlagged, mergedIncidents
    }), 'Save import batch record');

    document.getElementById('importResultBox').textContent =
`Import complete (${FILE_TYPE_LABELS[detectedFileType].split(' — ')[0]}).
...
Incidents flagged: ${incidentsFlagged}
Existing incidents updated with new details: ${mergedIncidents}
Rows skipped (no name, no order ID): ${skippedNoName}${orphanLedgerRows ? `
Ledger rows with no matching order yet: ${orphanLedgerRows} (import your Cancelled/Missing/Avoidable-Wait files — those carry the customer name — and re-upload this ledger file afterward to enrich them)` : ''}

Flagged customers will now show a yellow or red badge and appear in "View all defaulters" with full order details.`;
    document.getElementById('importStep2').style.display = 'none';
...
// Recomputes every customer's incidents from scratch, from the RAW fields already stored
// on each of their orders (isCancelledFlag, isMissingFlag, errorCharge, comment, status,
// errorCategory). This fixes cases where the incident logic was wrong or missing at import
// time, WITHOUT needing to re-upload — but it can't fix an order whose underlying field
// values were themselves wrong (e.g. an old import that put Subtotal into Error Charge);
// for that, delete the bad import from "Import history" and re-upload it.
function recomputeIncidentsForOrder(o){
  if(o.isCancelledFlag) return { issue:'Order cancelled' };
  if(o.isMissingFlag) return { issue: o.errorCategory || 'Missing / incorrect order' };
  if(parseFloat(o.errorCharge)||0) return { issue:'Error charge' };
  const kw = classifyOrderStatus(o.status||'', '', o.errorCategory||'');
  if(kw) return { issue: kw.issue };
  if(o.errorCategory) return { issue: o.errorCategory };
  if(o.comment) return { issue:'Customer comment on order' };
  return null;
}
document.getElementById('rescanDefaultersBtn').onclick = async ()=>{
  if(!confirm('Rescan every customer\'s existing orders and rebuild their incident list from scratch? This replaces current incidents with a fresh calculation based on the order data already on file.')) return;
  showLoading('Rescanning all customers…');
  try{
    const ops = [];
    let changedCount = 0, totalIncidents = 0;
    customers.forEach(c=>{
      const orders = c.orders || [];
      let incidents = [];
      orders.forEach(o=>{
        const classification = recomputeIncidentsForOrder(o);
        if(!classification) return;
        const inc = {
          date: o.date ? new Date(o.date).toISOString() : new Date().toISOString(),
          platform: o.restaurant||'', order: o.orderId||'', amount: o.errorCharge||o.amount||'',
          issue: classification.issue,
          notes: [o.comment, o.dasherName?`Dasher: ${o.dasherName}`:''].filter(Boolean).join(' — '),
          shots: [], importBatchId: o.importBatchId||''
        };
        const r = mergeIncidentIntoList(incidents, inc, c.name);
        incidents = r.incidents;
      });
      const before = JSON.stringify(c.incidents||[]);
      const after = JSON.stringify(incidents);
      if(before !== after){
        changedCount++;
        totalIncidents += incidents.length;
        const auditLog = c.auditLog || [];
        auditLog.push({ date:new Date().toISOString(), action:`Incidents rebuilt via "Rescan for defaulters" (${incidents.length} incident${incidents.length!==1?'s':''} now on file)` });
    ...
    toast(`✓ Rescan complete — ${changedCount} customer${changedCount!==1?'s':''} updated`, 'success');
  }catch(e){
...
function mergeCustomerGroup(group){
  // Keep whichever record has the most complete contact info as the "base"; merge the rest into it.
  const sorted = [...group].sort((a,b)=> (b.phone?1:0)+(b.email?1:0) - ((a.phone?1:0)+(a.email?1:0)));
  const base = sorted[0];
  let orders = [...(base.orders||[])];
  let incidents = [...(base.incidents||[])];
  let auditLog = [...(base.auditLog||[])];
  let phone = base.phone||'', email = base.email||'';
  const toDelete = [];
  sorted.slice(1).forEach(dupe=>{
    (dupe.orders||[]).forEach(o=>{ orders = mergeOrderIntoList(orders, o, base.name).orders; });
    (dupe.incidents||[]).forEach(inc=>{ incidents = mergeIncidentIntoList(incidents, inc, base.name).incidents; });
    auditLog = auditLog.concat(dupe.auditLog||[]);
    if(!phone && dupe.phone) phone = dupe.phone;
    if(!email && dupe.email) email = dupe.email;
    toDelete.push(dupe.id);
  });
  auditLog.push({ date:new Date().toISOString(), action:`Merged ${toDelete.length} duplicate profile${toDelete.length!==1?'s':''} into this one` });
  return { baseId: base.id, data:{ orders, incidents, auditLog, phone, email }, deleteIds: toDelete };
}
function renderMergeDupes(){
  const groups = findDuplicateGroups();
  const el = document.getElementById('mergeDupesList');
  el.innerHTML = groups.length ? groups.map((g,i)=>`
    <div class="card">
      <div style="font-weight:600;margin-bottom:6px;">${escapeHtml(g[0].name)} <span class="count-badge">${g.length} profiles</span></div>
      ${g.map(c=>`<div class="meta" style="font-size:12.5px;color:var(--text-dim);">${(c.orders||[]).length} orders, ${(c.incidents||[]).length} incidents${c.phone?' · '+escapeHtml(c.phone):''}${c.email?' · '+escapeHtml(c.email):''}</div>`).join('')}
      <button class="btn-primary" data-merge-group="${i}" style="margin-top:10px;">Merge these ${g.length} into one profile</button>
    </div>
  `).join('') : '<div class="empty">No duplicate names found right now.</div>';
  el.querySelectorAll('[data-merge-group]').forEach(btn=>{
    btn.onclick = async ()=>{
      const group = groups[parseInt(btn.dataset.mergeGroup)];
      if(!confirm(`Merge ${group.length} profiles named "${group[0].name}" into one? This can't be undone.`)) return;
      showLoading('Merging…');
      try{
        const { baseId, data, deleteIds } = mergeCustomerGroup(group);
        await withTimeout(db.collection('customers').doc(baseId).update(data), 'Merge update');
        for(const id of deleteIds) await withTimeout(db.collection('customers').doc(id).delete(), 'Merge cleanup delete');
        toast('✓ Profiles merged', 'success');
        renderMergeDupes();
      }catch(e){
...
document.getElementById('openMergeDupesBtn').onclick = ()=>{ renderMergeDupes(); document.getElementById('mergeDupesModal').classList.add('show'); };
document.getElementById('closeMergeDupesBtn').onclick = ()=> document.getElementById('mergeDupesModal').classList.remove('show');

// ================= IMPORT HISTORY / DELETE A SPECIFIC IMPORT =================
let importBatchesCache = [];
db.collection('importBatches').onSnapshot(snap=>{
  importBatchesCache = [];
  snap.forEach(doc=> importBatchesCache.push({ id: doc.id, ...doc.data() }));
  importBatchesCache.sort((a,b)=> new Date(b.importedAt||0) - new Date(a.importedAt||0));
}, err=> console.error('[importBatches] listener failed:', err));

async function deleteImportBatch(batchId){
  showLoading('Removing this import…');
  try{
    const ops = [];
    customers.forEach(c=>{
      const orders = (c.orders||[]).filter(o=> o.importBatchId !== batchId);
      const incidents = (c.incidents||[]).filter(i=> i.importBatchId !== batchId);
      if(orders.length !== (c.orders||[]).length || incidents.length !== (c.incidents||[]).length){
        const auditLog = c.auditLog || [];
        auditLog.push({ date:new Date().toISOString(), action:'Data from a deleted import was removed from this profile' });
        ops.push({ ref: db.collection('customers').doc(c.id), data:{ orders, incidents, auditLog } });
      }
    });
    for(let i=0;i<ops.length;i+=400){
      const batch = db.batch();
      ops.slice(i,i+400).forEach(op=> batch.update(op.ref, op.data));
      await withTimeout(batch.commit(), 'Delete-batch commit');
    }
    await withTimeout(db.collection('importBatches').doc(batchId).delete(), 'Delete import record');
    toast(`✓ Import removed from ${ops.length} customer${ops.length!==1?'s':''}`, 'success');
  }catch(e){
    console.error('[deleteImportBatch] Failed:', e);
    toast('Could not remove this import: '+e.message, 'error');
  }finally{
    hideLoading();
  }
}
function renderImportHistory(){
  ...
  el.innerHTML = importBatchesCache.length ? importBatchesCache.map(b=>`
    <div class="session-row">
      <div class="top"><span>${escapeHtml(b.fileName||'Unnamed file')}</span><span>${fmtDate(b.importedAt)}</span></div>
      <div class="meta">${escapeHtml((FILE_TYPE_LABELS[b.fileType]||'').split(' — ')[0]||b.fileType||'')} · ${b.newOrders||0} new orders, ${b.incidentsFlagged||0} incidents flagged, ${b.customersCreated||0} new customers</div>
      <button class="btn-danger" data-delete-batch="${b.id}" style="margin-top:8px;">Delete this import's data</button>
    </div>
  `).join('') : '<div class="empty">No imports recorded yet.</div>';
  el.querySelectorAll('[data-delete-batch]').forEach(btn=>{
    btn.onclick = async ()=>{
      if(!confirm('Remove every order and incident that came from this specific file, from all customers it touched? This cannot be undone.')) return;
      await deleteImportBatch(btn.dataset.deleteBatch);
      renderImportHistory();
    };
  });
}
document.getElementById('openImportHistoryBtn').onclick = ()=>{ renderImportHistory(); document.getElementById('importHistoryModal').classList.add('show'); };
document.getElementById('closeImportHistoryBtn').onclick = ()=> document.getElementById('importHistoryModal').classList.remove('show');
...
let groceryActiveCategory = 'all';
function renderGrocery(){
  const q = normalize(document.getElementById('grocerySearch').value);
  const sorted = [...groceryItems]
    .filter(i=> !q || normalize(i.name).includes(q) || normalize(i.category||'').includes(q) || normalize(i.notes||'').includes(q))
    .filter(i=> groceryActiveCategory==='all' || categoryBucket(i.category)===groceryActiveCategory)
    .sort((a,b)=>(a.name||'').localeCompare(b.name||''));

  const filterWrap = document.getElementById('groceryCatFilters');
  const catsPresent = CATEGORY_ORDER.filter(cat=> groceryItems.some(i=>categoryBucket(i.category)===cat));
  filterWrap.innerHTML = ['all', ...catsPresent].map(cat=>
    `<button class="btn-outline${cat===groceryActiveCategory?' active-filter':''}" data-cat-filter="${escapeHtml(cat)}" style="font-size:12px;padding:6px 12px;${cat===groceryActiveCategory?'background:var(--navy);color:#fff;border-color:var(--navy);':''}">${cat==='all'?'All categories':escapeHtml(cat)}</button>`
  ).join('');
  filterWrap.querySelectorAll('[data-cat-filter]').forEach(btn=>{
    btn.onclick = ()=>{ groceryActiveCategory = btn.getAttribute('data-cat-filter'); renderGrocery(); };
  });

  if(!sorted.length){
    document.getElementById('generalGroceryList').innerHTML = `<div class="empty">${groceryItems.length ? 'No items match your search/filter.' : 'No items in your grocery catalog yet. Tap "Add item" to start one.'}</div>`;
  }else{
    let html = '';
  ...
  return `<div class="grocery-item">
    ...
    <div style="display:flex;flex-direction:column;gap:6px;align-items:stretch;">
      ...
      <div style="display:flex;gap:4px;flex-wrap:wrap;">
        <button class="btn-outline" data-grocery-edit="${item.id}" style="font-size:12px;padding:6px 9px;">Edit</button>
        <button class="btn-outline" data-grocery-fav="${item.id}" style="font-size:12px;padding:6px 9px;">${item.favorite?'★ Favorited':'☆ Favorite'}</button>
        <button class="btn-outline" data-grocery-dup="${item.id}" style="font-size:12px;padding:6px 9px;">Duplicate</button>
        <button class="btn-danger" data-grocery-delete="${item.id}" style="font-size:12px;padding:6px 9px;">Delete</button>
      </div>
    </div>
  </div>`;
}
...
document.getElementById('grocerySearch').addEventListener('input', renderGrocery);
...
// ================= REWARDS VAULT =================
// Secret-code redemption system. Codes and their reward tiers live in Firestore
// (collections `vaultCodes` and `vaultConfig`), gated by the same Firebase Auth
// login as the rest of this app. Nothing about which tier a code is worth is
// ever computed or stored client-side only — it's read from the database, so a
// customer can't view-source or edit browser storage to fake a win.
const DEFAULT_VAULT_TIERS = [
  { heat:'🌶️', name:'Mild', reward:'Free Naan', weight:40 },
  { heat:'🌶️🌶️', name:'Medium', reward:'Free Drink', weight:28 },
  { heat:'🌶️🌶️🌶️', name:'Hot', reward:'25% Off Next Order', weight:18 },
  { heat:'🌶️🌶️🌶️🌶️', name:'Extra Hot', reward:'Free Appetizer', weight:10 },
  { heat:'🔥', name:'THALI JACKPOT', reward:'Free Meal for Two', weight:4 },
];
const VAULT_CHARSET = 'ABCDEFGHJKMNPQRSTUVWXYZ23456789'; // no 0/O/1/I — avoids customer misreads
// Where a scanned QR code should land. Point this at wherever your public
// Spice Vault page (spice-vault.html / rewards.html) actually lives —
// your real domain, not a github.io link, so it looks trustworthy on a printed coupon.
const VAULT_PUBLIC_URL = 'https://advsurinder.github.io/newfilesnwdf-qefq/spice-vault.html';
let vaultCodes = [];
let vaultConfig = { tiers: DEFAULT_VAULT_TIERS, streakN: 5 };
let vaultLastBatch = [];

function vaultRandCode(){
  let s = 'S17-';
  for(let i=0;i<5;i++) s += VAULT_CHARSET[Math.floor(Math.random()*VAULT_CHARSET.length)];
  return s;
}
function vaultPickTier(){
  const tiers = vaultConfig.tiers;
  const total = tiers.reduce((a,t)=>a+Number(t.weight||0),0);
  let r = Math.random()*total;
  for(const t of tiers){ r -= Number(t.weight||0); if(r<=0) return t; }
  return tiers[0];
}

// ---- Generate & print ----
document.getElementById('v-genBtn').onclick = async ()=>{
  const count = Math.max(1, Math.min(500, Number(document.getElementById('v-genCount').value)||1));
  const label = document.getElementById('v-genLabel').value.trim();
  const btn = document.getElementById('v-genBtn');
  btn.disabled = true;
  const originalText = btn.innerHTML;
  btn.innerHTML = 'Generating…';
  try{
    const existing = new Set(vaultCodes.map(c=>c.code));
    const batch = db.batch();
    const created = [];
    for(let i=0;i<count;i++){
      let code;
      do{ code = vaultRandCode(); } while(existing.has(code));
      existing.add(code);
      const tier = vaultPickTier();
      const entry = { code, tierName:tier.name, reward:tier.reward, heat:tier.heat, redeemed:false, batch:label, createdAt:new Date().toISOString() };
      batch.set(db.collection('vaultCodes').doc(code), entry);
      created.push(entry);
    }
    console.log('[vault] Writing', count, 'codes');
    await withTimeout(batch.commit(), 'Generate codes');
    console.log('[vault] Codes committed ✓');
    vaultLastBatch = created;
    document.getElementById('v-printBtn').disabled = false;
    document.getElementById('v-pdfBtn').disabled = false;
    vaultRenderGenGrid(created);
    toast(`${count} codes generated`, 'success');
  }catch(e){
    console.error('[vault] Generate failed:', e);
...
function vaultRenderGenGrid(batch){
  document.getElementById('v-genGrid').innerHTML = batch.map(c=>`
    <div class="v-code-card"><div class="seal"><i class="ti ti-lock" aria-hidden="true"></i></div><div class="code">${c.code}</div><div class="hint">reward hidden until unlocked</div></div>
  `).join('');
}
// ---- Coupon design: shared by both the print view and the PDF export ----
async function vaultQrDataUrl(code, sizePx){
  if(typeof QRCode === 'undefined'){
    throw new Error('QR code library did not load — check that index.html includes the qrcode.min.js <script> tag and that nothing is blocking cdnjs.cloudflare.com.');
  }
  const url = `${VAULT_PUBLIC_URL}?code=${encodeURIComponent(code)}`;
  return QRCode.toDataURL(url, { margin:0, width:sizePx || 220, color:{ dark:'#141414', light:'#ffffff' } });
}

// Waits for every <img> inside a container (logo + QR) to finish loading
// before we let the browser open the print dialog — printing before images
// decode is the #1 reason a coupon prints as an empty box.
function vaultWaitForImages(container){
...
document.getElementById('v-printBtn').onclick = async ()=>{
  if(!vaultLastBatch.length){ toast('Generate a batch first', 'error'); return; }
  const btn = document.getElementById('v-printBtn');
  btn.disabled = true; const original = btn.innerHTML; btn.innerHTML = 'Preparing…';
  try{
    const style = document.getElementById('v-couponStyle').value || 'classic';
    const cardsHtml = await Promise.all(vaultLastBatch.map(async c=>{
      const qr = await vaultQrDataUrl(c.code, 220);
      return vaultCouponCardHtml(c, qr, style);
    }));
    const grid = document.getElementById('v-printGrid');
    grid.innerHTML = cardsHtml.join('');

    await vaultWaitForImages(grid); // <-- don't print before logo/QR images finish decoding
    window.print();
  }catch(e){
    console.error('[vault] Print prep failed:', e);
    toast('Could not prepare coupons: '+e.message, 'error');
  }finally{
    btn.disabled = false; btn.innerHTML = original;
  }
};

// Builds the printable markup for one coupon in the chosen visual style.
// 'classic' and 'ribbon' share the same stacked layout (just different CSS,
// via the style-ribbon class); 'ticket' needs its own two-column markup for
// the tear-off stub.
function vaultCouponCardHtml(c, qr, style){
  const note = `Scan the code, or enter it at checkout.<br>One-time use — verified by staff.`;
  if(style === 'ticket'){
    return `
      <div class="v-print-card style-ticket">
        <div class="pc-main">
          <img class="pc-logo" src="logo-dark-for-coupons.png" alt="The Sector 17">
          <div class="pc-eyebrow">A Secret Reward</div>
          <div class="pc-divider"></div>
          <div class="pc-code">${c.code}</div>
          <img class="pc-qr" src="${qr}" alt="QR code">
          <div class="pc-note">${note}</div>
        </div>
        <div class="pc-stub"><div class="stub-code">${c.code}</div></div>
      </div>`;
  }
  if(style === 'ribbon'){
    return `
      <div class="v-print-card style-ribbon">
        <div class="pc-banner"><div class="pc-eyebrow">A Secret Reward</div></div>
        <img class="pc-logo" src="logo-dark-for-coupons.png" alt="The Sector 17">
        <div class="pc-divider"></div>
        <div class="pc-code">${c.code}</div>
        <img class="pc-qr" src="${qr}" alt="QR code">
...
document.getElementById('v-pdfBtn').onclick = async ()=>{
  if(!vaultLastBatch.length){ toast('Generate a batch first', 'error'); return; }
  if(!window.jspdf){ toast('PDF library did not load — check index.html includes the jspdf <script> tag and try reloading.', 'error'); return; }
  const btn = document.getElementById('v-pdfBtn');
  btn.disabled = true; const original = btn.innerHTML; btn.innerHTML = 'Building PDF…';
  try{
    const style = document.getElementById('v-couponStyle').value || 'classic';
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ unit:'mm', format:'a4' });
    const logoImgEl = document.getElementById('couponLogoImg');
    await new Promise(res=>{
      if(logoImgEl.complete){ res(); return; }
      logoImgEl.addEventListener('load', res, { once:true });
      logoImgEl.addEventListener('error', res, { once:true });
      setTimeout(res, 3000);
    });
    const logoOk = logoImgEl.naturalWidth > 0;
    if(!logoOk) console.warn('[vault] logo-dark-for-coupons.png not found or failed to load — PDF will be generated without it.');
    const logoImg = logoOk ? logoImgEl : null;
    const logoRatio = logoOk ? (logoImgEl.naturalHeight / logoImgEl.naturalWidth) : 0.27;

    const marginX = 10, marginY = 12, gap = 6, perPage = 6;
    const cw = (210 - marginX*2 - gap) / 2;
    const ch = (297 - marginY*2 - gap*2) / 3;

    for(let i=0;i<vaultLastBatch.length;i++){
      const posInPage = i % perPage;
      if(i>0 && posInPage===0) doc.addPage();
      const col = posInPage % 2, row = Math.floor(posInPage/2);
      const x = marginX + col*(cw+gap), y = marginY + row*(ch+gap);
      const c = vaultLastBatch[i];
      const qrDataUrl = await vaultQrDataUrl(c.code, 240);

      if(style === 'ticket') drawCouponTicket(doc, c, qrDataUrl, x, y, cw, ch, logoImg, logoRatio);
      else if(style === 'ribbon') drawCouponRibbon(doc, c, qrDataUrl, x, y, cw, ch, logoImg, logoRatio);
      else drawCouponClassic(doc, c, qrDataUrl, x, y, cw, ch, logoImg, logoRatio);
    }

    doc.save('the-sector-17-vault-coupons.pdf');
  }catch(e){
    console.error('[vault] PDF export failed:', e);
...
const VAULT_PDF_FOOTER_1 = 'Scan the code, or enter it at checkout.';
const VAULT_PDF_FOOTER_2 = 'One-time use — verified at checkout.';

function drawCouponClassic(doc, c, qrDataUrl, x, y, cw, ch, logoImg, logoRatio){
  doc.setLineDashPattern([1.4,1.4], 0);
  doc.setDrawColor(60,60,60);
  doc.roundedRect(x, y, cw, ch, 2, 2);
  doc.setLineDashPattern([], 0);

  const logoW = 26, logoH = logoW * logoRatio;
  try{ doc.addImage(logoImg, 'PNG', x+cw/2-logoW/2, y+4, logoW, logoH); }catch(e){}

  doc.setFont('helvetica','bold'); doc.setFontSize(7.2); doc.setTextColor(90,90,90);
  doc.text('A SECRET REWARD', x+cw/2, y+logoH+9, { align:'center', charSpace:0.5 });

  doc.setDrawColor(170,170,170);
  doc.line(x+cw/2-15, y+logoH+12, x+cw/2+15, y+logoH+12);

  doc.setFont('courier','bold'); doc.setFontSize(16); doc.setTextColor(15,15,15);
  doc.text(c.code, x+cw/2, y+logoH+23, { align:'center' });

  const qrSize = 21;
  doc.addImage(qrDataUrl, 'PNG', x+cw/2-qrSize/2, y+logoH+27, qrSize, qrSize);

  doc.setFont('helvetica','normal'); doc.setFontSize(6.1); doc.setTextColor(100,100,100);
  doc.text(VAULT_PDF_FOOTER_1, x+cw/2, y+ch-8, { align:'center' });
  doc.text(VAULT_PDF_FOOTER_2, x+cw/2, y+ch-4.5, { align:'center' });
}

function drawCouponRibbon(doc, c, qrDataUrl, x, y, cw, ch, logoImg, logoRatio){
  doc.setDrawColor(0,0,0); doc.setLineWidth(0.5);
  doc.roundedRect(x, y, cw, ch, 2, 2);

  // Solid black banner across the top — the highest-contrast element on a B&W printer
  const bannerH = 9;
  doc.setFillColor(0,0,0);
  doc.rect(x, y, cw, bannerH, 'F');
  doc.setFont('helvetica','bold'); doc.setFontSize(7.4); doc.setTextColor(255,255,255);
  doc.text('A SECRET REWARD', x+cw/2, y+bannerH/2+1.3, { align:'center', charSpace:0.6 });

  const logoW = 24, logoH = logoW * logoRatio;
  try{ doc.addImage(logoImg, 'PNG', x+cw/2-logoW/2, y+bannerH+3, logoW, logoH); }catch(e){}

  doc.setDrawColor(0,0,0); doc.setLineWidth(0.35);
  doc.line(x+cw/2-15, y+bannerH+logoH+7, x+cw/2+15, y+bannerH+logoH+7);

  doc.setFont('courier','bold'); doc.setFontSize(15.5); doc.setTextColor(0,0,0);
  doc.setDrawColor(0,0,0); doc.setLineWidth(0.35);
  const codeY = y+bannerH+logoH+18;
  doc.roundedRect(x+cw/2-19, codeY-5.5, 38, 8, 1, 1);
  doc.text(c.code, x+cw/2, codeY, { align:'center' });

  const qrSize = 20;
  doc.addImage(qrDataUrl, 'PNG', x+cw/2-qrSize/2, codeY+7, qrSize, qrSize);

  doc.setFont('helvetica','normal'); doc.setFontSize(6.1); doc.setTextColor(80,80,80);
  doc.text(VAULT_PDF_FOOTER_1, x+cw/2, y+ch-8, { align:'center' });
  doc.text(VAULT_PDF_FOOTER_2, x+cw/2, y+ch-4.5, { align:'center' });
}

function drawCouponTicket(doc, c, qrDataUrl, x, y, cw, ch, logoImg, logoRatio){
  const stubW = 12;
  const mainW = cw - stubW;

  doc.setDrawColor(30,30,30); doc.setLineWidth(0.45);
  doc.rect(x, y, cw, ch);

  // Perforated divider between the main coupon and the tear-off stub
  doc.setLineDashPattern([1,1], 0);
  doc.setDrawColor(90,90,90);
  doc.line(x+mainW, y, x+mainW, y+ch);
  doc.setLineDashPattern([], 0);
  doc.setFillColor(255,255,255);
  doc.setDrawColor(90,90,90); doc.setLineWidth(0.3);
  doc.circle(x+mainW, y, 1.6, 'FD');
  doc.circle(x+mainW, y+ch, 1.6, 'FD');

  const logoW = 22, logoH = logoW * logoRatio;
  try{ doc.addImage(logoImg, 'PNG', x+mainW/2-logoW/2, y+4, logoW, logoH); }catch(e){}

  doc.setFont('helvetica','bold'); doc.setFontSize(7); doc.setTextColor(90,90,90);
  doc.text('A SECRET REWARD', x+mainW/2, y+logoH+8, { align:'center', charSpace:0.5 });

  doc.setFont('courier','bold'); doc.setFontSize(14); doc.setTextColor(15,15,15);
  doc.text(c.code, x+mainW/2, y+logoH+18, { align:'center' });

  const qrSize = 19;
  doc.addImage(qrDataUrl, 'PNG', x+mainW/2-qrSize/2, y+logoH+22, qrSize, qrSize);

  doc.setFont('helvetica','normal'); doc.setFontSize(5.8); doc.setTextColor(100,100,100);
  doc.text(VAULT_PDF_FOOTER_1, x+mainW/2, y+ch-8, { align:'center', maxWidth: mainW-4 });
  doc.text(VAULT_PDF_FOOTER_2, x+mainW/2, y+ch-4.5, { align:'center' });

  // Vertical code repeated on the tear-off stub
  doc.setFont('courier','bold'); doc.setFontSize(9); doc.setTextColor(15,15,15);
  doc.text(c.code, x+mainW+stubW/2, y+ch/2, { align:'center', angle:90 });
}

// ---- Redeem ----
async function vaultDoRedeem(){
  const codeInput = document.getElementById('v-redeemCode');
  const raw = codeInput.value.trim().toUpperCase();
  const channel = document.getElementById('v-redeemChannel').value;
  const phone = document.getElementById('v-redeemPhone').value.trim();
  const resultBox = document.getElementById('v-redeemResult');
  const btn = document.getElementById('v-lookupBtn');
  if(!raw){
    resultBox.innerHTML = `<div class="empty">Enter a code first — type or paste it exactly as printed.</div>`;
    return;
  }
  btn.disabled = true;
  const originalText = btn.innerHTML;
  ...
  try{
    let jackpotOverride = false;
    const topTier = vaultConfig.tiers[vaultConfig.tiers.length-1];
    if(phone){
      const priorSnap = await withTimeout(db.collection('vaultCodes').where('phone','==',phone).get(), 'Streak lookup');
      const priorCount = priorSnap.docs.filter(d=>d.data().redeemed).length;
      if((priorCount+1) % Number(vaultConfig.streakN||5) === 0) jackpotOverride = true;
    }

    const ref = db.collection('vaultCodes').doc(raw);
    const outcome = await withTimeout(db.runTransaction(async (tx)=>{
      const snap = await tx.get(ref);
      if(!snap.exists) return { status:'notfound' };
      const data = snap.data();
      if(data.redeemed) return { status:'already', data };
      const finalTier = jackpotOverride
        ? { tierName: topTier.name, reward: topTier.reward, heat: topTier.heat }
        : { tierName: data.tierName, reward: data.reward, heat: data.heat };
      const redeemedAt = new Date().toISOString();
      tx.update(ref, { redeemed:true, redeemedAt, channel, phone: phone||null, ...finalTier });
      return { status:'ok', data:{ ...data, ...finalTier, redeemed:true, redeemedAt, channel, phone } };
    }), 'Redeem code');

    if(outcome.status==='notfound'){
      resultBox.innerHTML = `<div class="empty">No match for <b>${escapeHtml(raw)}</b> — double-check the spelling. Codes skip 0/O and 1/I.</div>`;
    }else if(outcome.status==='already'){
      resultBox.innerHTML = `<div class="empty">Already unlocked on ${fmtDate(outcome.data.redeemedAt)}. Each code works once.</div>`;
    }else{
      const e = outcome.data;
      const waMsg = `THE SECTOR 17 — REWARDS VAULT\n\nCode ${e.code} unlocked!\nYou won: ${e.heat} ${e.tierName} — ${e.reward}\n\nShow this message to redeem in-store, or mention it on your next order.${jackpotOverride ? `\n\nBonus: this was your streak reward — every ${vaultConfig.streakN}th code is a guaranteed jackpot!` : ''}`;
      resultBox.innerHTML = `
        <div class="v-reveal">
          <div class="heat">${e.heat}</div>
          <div class="tier-label">${escapeHtml(e.tierName)}${jackpotOverride?' · STREAK BONUS':''}</div>
          <div class="reward-name">${escapeHtml(e.reward)}</div>
        </div>
        <div class="v-wa-box" id="v-waTemplate"></div>
        <div style="margin-top:10px;"><button class="btn-outline" id="v-copyWaBtn">Copy WhatsApp reply</button></div>
      `;
      document.getElementById('v-waTemplate').textContent = waMsg;
      document.getElementById('v-copyWaBtn').onclick = ()=>{
        navigator.clipboard.writeText(waMsg).then(()=>toast('Reply copied — paste it in WhatsApp','success'));
      };
      codeInput.value = '';
      toast('Code unlocked', 'success');
    }
  }catch(e){
    console.error('[vault] Redeem failed:', e);
    resultBox.innerHTML = `<div class="empty">Something went wrong: ${escapeHtml(e.message)}</div>`;
  }finally{
    btn.disabled = false;
    btn.innerHTML = originalText;
  }
}
document.getElementById('v-lookupBtn').onclick = vaultDoRedeem;
document.getElementById('v-redeemCode').addEventListener('keydown', e=>{ if(e.key==='Enter') vaultDoRedeem(); });

// ---- Dashboard ----
function vaultRenderDashboard(){
  const total = vaultCodes.length;
  const redeemed = vaultCodes.filter(c=>c.redeemed).length;
  const rate = total ? Math.round((redeemed/total)*100) : 0;
  const stamp = document.getElementById('vaultRateStamp');
  if(stamp) stamp.textContent = rate + '% redeemed';
  document.getElementById('v-statGrid').innerHTML = `
    <div class="stat-chip"><div class="n">${total}</div><div class="l">Codes printed</div></div>
    <div class="stat-chip"><div class="n">${redeemed}</div><div class="l">Unlocked</div></div>
    <div class="stat-chip"><div class="n">${rate}%</div><div class="l">Redemption rate</div></div>
  `;
  const recent = vaultCodes.filter(c=>c.redeemed).sort((a,b)=> new Date(b.redeemedAt)-new Date(a.redeemedAt)).slice(0,25);
  document.getElementById('v-logBody').innerHTML = recent.map(c=>`
    <tr><td style="font-family:var(--mono);">${c.code}</td><td>${c.heat||''} ${escapeHtml(c.reward||'')}</td><td>${c.channel||'—'}</td><td>${fmtDate(c.redeemedAt)}</td></tr>
  `).join('') || `<tr><td colspan="4" class="empty">No redemptions yet.</td></tr>`;
}

// ---- Reward tier & streak settings ----
function vaultRenderTierRows(){
  const box = document.getElementById('v-tierRows');
  box.innerHTML = `<div class="v-tier-header"><span>Heat</span><span>Tier name</span><span>Reward</span><span>Weight</span><span></span></div>` +
    vaultConfig.tiers.map((t,i)=>`
      <div class="v-tier-row">
        <input type="text" value="${escapeHtml(t.heat)}" data-vfield="heat" data-vi="${i}">
        <input type="text" value="${escapeHtml(t.name)}" data-vfield="name" data-vi="${i}">
        <input type="text" value="${escapeHtml(t.reward)}" data-vfield="reward" data-vi="${i}">
        <input type="number" value="${t.weight}" data-vfield="weight" data-vi="${i}">
        <button class="v-tier-remove" data-vremove="${i}" title="Remove tier">✕</button>
      </div>
    `).join('');
  box.querySelectorAll('input').forEach(inp=>{
    inp.addEventListener('input', e=>{
      const i = e.target.dataset.vi, f = e.target.dataset.vfield;
      vaultConfig.tiers[i][f] = f==='weight' ? Number(e.target.value) : e.target.value;
    });
  });
  box.querySelectorAll('[data-vremove]').forEach(b=>{
    b.addEventListener('click', e=>{
      vaultConfig.tiers.splice(Number(e.target.dataset.vremove),1);
      vaultRenderTierRows();
    });
  });
}
document.getElementById('v-addTierBtn').onclick = ()=>{
  vaultConfig.tiers.push({ heat:'🌶️', name:'New tier', reward:'Reward name', weight:10 });
  vaultRenderTierRows();
};
document.getElementById('v-saveTiersBtn').onclick = async ()=>{
  try{
    await withTimeout(db.collection('vaultConfig').doc('settings').set(vaultConfig), 'Save tiers');
    toast('Reward tiers saved', 'success');
  }catch(e){ console.error('[vault] Save tiers failed:', e); toast('Save failed: '+e.message, 'error'); }
};
document.getElementById('v-saveStreakBtn').onclick = async ()=>{
  vaultConfig.streakN = Math.max(2, Number(document.getElementById('v-streakN').value)||5);
  try{
    await withTimeout(db.collection('vaultConfig').doc('settings').set(vaultConfig), 'Save streak setting');
    toast('Streak setting saved', 'success');
  }catch(e){ console.error('[vault] Save streak failed:', e); toast('Save failed: '+e.message, 'error'); }
};
...
async function handleIncomingOrder(orderDoc){
  ...
  if(!name){
    ...
    // Show a snippet of whatever text WAS captured (email body + any OCR'd PDF text) so
    // it's possible to tell, from the popup alone, whether the problem is "no PDF was
    // read at all" (snippet looks like a generic email) vs "PDF was read but the name
    // pattern didn't match" (snippet has real order text, just not in the expected shape).
    const snippet = (p.itemsSummary || '').trim().slice(0, 90).replace(/\s+/g,' ');
    showOrderPopup({
      title:'Unrecognized order email', isNew:false, flagged:true, name:'Needs manual review',
      meta: snippet ? `Captured text starts: "${snippet}${p.itemsSummary && p.itemsSummary.length>90?'…':''}" — open the live feed for the full text.` : `No readable text at all — likely the PDF wasn't OCR'd (check the Drive API is enabled in Apps Script).`
    });
    return;
  }
...
function renderOrderFeed(){
  document.getElementById('orderFeedList').innerHTML = orderFeedItems.length ? orderFeedItems.slice(0,60).map(o=>{
    ...
    return `<div class="feed-row">
      ...
      ${!p.name && p.itemsSummary ? `<div class="meta" style="white-space:pre-wrap;background:var(--paper);padding:8px;border-radius:6px;margin-top:6px;max-height:120px;overflow-y:auto;">${escapeHtml(p.itemsSummary.slice(0,600))}</div>` : ''}
