...
function custRowHtml(c){
  ...
  return `<div class="cust-row" data-cust-id="${c.id}">
    <div>
      <div style="margin-bottom:4px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">${safetyBadgeHtml(c)}${rest?`<span class="rest-tag">${escapeHtml(rest)}</span>`:''}</div>
...
function showCustomerDetail(id){
  ...
  document.getElementById('custDetail').innerHTML = `
    <div class="card">
      ${stamp}
      <div style="margin:10px 0 4px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">${safetyBadgeHtml(c)}${primaryRestaurant(c)?`<span class="rest-tag">${escapeHtml(primaryRestaurant(c))}</span>`:''}</div>
...
document.getElementById('runImportBtn').onclick = async ()=>{
  ...
  if(colByField.status === undefined && colByField.errorCategory === undefined && colByField.comment === undefined){
  ...
  try{
    ...
    importRows.forEach(row=>{
      ...
      // Any explicit status/error-category keyword flags it. If there's no keyword match
      // but the row still has a customer comment or a nonzero error charge, treat that as
      // a defaulter signal too — that's how these spreadsheets mark problem orders even
      // without a dedicated status column (a blank comment = clean order, by convention).
