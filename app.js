    ...
    const cardsHtml = await Promise.all(vaultLastBatch.map(async c=>{
      ...
      return `
        <div class="v-print-card">
          ...
          <div class="pc-note">Scan, or enter this code at <strong>thesector17.com/rewards</strong>.<br>One-time use — verified at checkout.</div>
        </div>`;
    }));
...
document.getElementById('v-pdfBtn').onclick = async ()=>{
  ...
  try{
    ...
    for(let i=0;i<vaultLastBatch.length;i++){
      ...
      try{ doc.addImage(logoImg, 'PNG', x+cw/2-logoW/2, y+4, logoW, logoH); }catch(e){ /* logo optional */ }
      ...
      doc.text('Scan, or enter this code at thesector17.com/rewards.', x+cw/2, y+ch-8, { align:'center' });
      doc.text('One-time use — verified at checkout.', x+cw/2, y+ch-4.5, { align:'center' });
    }
