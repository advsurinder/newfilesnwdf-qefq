...
function checkAutoReorder(){
  ...
  groceryItems.forEach(async item=>{
    ...
    if(new Date() >= nextDue){
      try{
        await db.collection('groceryItems').doc(item.id).update({ inCart:true, cartQty: item.cartQty||1, autoAdded:true });
...
function renderGrocery(){
  ...
  document.getElementById('generalGroceryList').innerHTML = sorted.length
    ? sorted.map(generalGroceryRowHtml).join('')
    : '<div class="empty">No items in your grocery catalog yet. Tap "Add item" to start one.</div>';

  document.querySelectorAll('[data-cart-add]').forEach(btn=>{
    btn.onclick = async ()=>{
      try{ await db.collection('groceryItems').doc(btn.getAttribute('data-cart-add')).update({ inCart:true, cartQty:1, autoAdded:false }); }
  ...
  document.querySelectorAll('[data-cart-remove-general]').forEach(btn=>{
    btn.onclick = async ()=>{
      try{ await db.collection('groceryItems').doc(btn.getAttribute('data-cart-remove-general')).update({ inCart:false }); }
  ...
  document.querySelectorAll('[data-grocery-delete]').forEach(btn=>{
    btn.onclick = async ()=>{
      ...
      try{ await db.collection('groceryItems').doc(btn.getAttribute('data-grocery-delete')).delete(); }
...
function renderCart(){
  ...
  document.querySelectorAll('[data-cart-qty]').forEach(inp=>{
    inp.onchange = async ()=>{
      ...
      try{ await db.collection('groceryItems').doc(inp.getAttribute('data-cart-qty')).update({ cartQty: qty }); }
      catch(e){ console.error('Qty update failed:', e); toast('Failed to update quantity: '+e.message, 'error'); }
    };
  });
  document.querySelectorAll('[data-cart-remove]').forEach(btn=>{
    btn.onclick = async ()=>{
      try{ await db.collection('groceryItems').doc(btn.getAttribute('data-cart-remove')).update({ inCart:false }); }
      catch(e){ console.error('Remove from cart failed:', e); toast('Failed: '+e.message, 'error'); }
    };
  });
...
document.getElementById('completeCartBtn').onclick = async ()=>{
  ...
  try{
    for(const item of cartItems){
      ...
      await db.collection('groceryItems').doc(item.id).update({
...
document.getElementById('downloadCartPdfBtn').onclick = ()=>{
  ...
  const doc = new jsPDF();
  doc.setFontSize(14);
  doc.text('Jiox Restaurant Manager — Grocery Receipt', 14, 16);
  doc.setFontSize(9);
  doc.text('Generated ' + new Date().toLocaleString(), 14, 22);
  let y = 34;
  doc.setFontSize(10);
  doc.text('Item', 14, y); doc.text('Qty', 120, y); doc.text('Price', 145, y); doc.text('Subtotal', 175, y);
  y += 4;
  doc.line(14, y, 196, y);
  y += 6;
  let total = 0;
  cartItems.forEach(item=>{
    ...
    const sub = price*qty;
    total += sub;
    doc.text(String(item.name).slice(0,45), 14, y);
    doc.text(String(qty), 120, y);
    doc.text(price?('$'+price.toFixed(2)):'—', 145, y);
    doc.text('$'+sub.toFixed(2), 175, y);
    y += 6;
  });
  y += 4;
  doc.line(14, y, 196, y);
  ...
  doc.save('jiox-grocery-receipt.pdf');
};
...
document.getElementById('saveGroceryBtn').onclick = async ()=>{
  ...
  }catch(e){ console.error('Grocery save failed:', e); toast('Save failed: '+e.message, 'error'); }
...
document.getElementById('saveExpenseBtn').onclick = async ()=>{
  ...
  }catch(e){ console.error('Expense save failed:', e); toast('Save failed: '+e.message, 'error'); }
...
document.getElementById('saveNoteBtn').onclick = async ()=>{
  ...
  }catch(e){ console.error('Note save failed:', e); toast('Save failed: '+e.message, 'error'); }
};
