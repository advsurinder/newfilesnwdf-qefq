...
document.getElementById('runImportBtn').onclick = async ()=>{
  ...
  try{
    ...
    importRows.forEach(row=>{
      ...
      // Only fall back to reading the comment column for status keywords if there's no explicit status/error-category column.
      const classification = classifyOrderStatus(statusText, (statusText||errorCategory) ? '' : comment, errorCategory);
      let incident = null;
      if(classification){
        const noteParts = [];
