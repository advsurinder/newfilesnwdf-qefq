...
// apply retroactively without re-uploading any Excel file. It only ADDS incidents that are missing;
// it never removes ones you've already logged.
function rebuildIncidentsFromOrders(c){
  ...
  orders.forEach(o=>{
    ...
    const hasErrorChargeCell = o.errorCharge !== undefined && o.errorCharge !== null && o.errorCharge !== '';
    let classification = classifyOrderStatus(o.status||'', (o.status||o.errorCategory) ? '' : (o.comment||''), o.errorCategory||'');
    if(!classification && (o.comment || hasErrorChargeCell)){
      classification = { type:'flagged', issue: errorChargeNum ? 'Error charge' : (o.comment ? 'Customer comment on order' : 'Error charge (logged as $0)') };
    }
...
async function runDefaulterRescan(){
  ...
  try{
    const ops = [];
    let customersUpdated = 0, incidentsAdded = 0;
    customers.forEach(c=>{
      if(!(c.orders||[]).length) return;
      const { incidents, added } = rebuildIncidentsFromOrders(c);
      if(added > 0){
        const auditLog = [...(c.auditLog||[])];
        auditLog.push({ date:new Date().toISOString(), action:`Rescan found ${added} additional flagged order${added>1?'s':''} from existing order history — now ${incidents.length} case${incidents.length===1?'':'s'} on file` });
        ops.push({ ref: db.collection('customers').doc(c.id), data:{ incidents, auditLog } });
        customersUpdated++; incidentsAdded += added;
      }
    });
    if(!ops.length){
      toast('Rescan complete — no missed defaulter orders found. All flags are already up to date.', 'success');
      return;
    }
    ...
    toast(`✓ Rescan complete — ${customersUpdated} customer(s) updated, ${incidentsAdded} defaulter case(s) recovered from order history`, 'success');
    if(lastSearchRan) runCustomerSearch();
  }catch(e){
...
document.getElementById('runImportBtn').onclick = async ()=>{
  ...
  try{
    ...
    importRows.forEach(row=>{
      ...
      if(!classification && (comment || errorChargeRaw !== '')){
        classification = { type:'flagged', issue: errorCharge ? 'Error charge' : (comment ? 'Customer comment on order' : 'Error charge (logged as $0)') };
      }
      let incident = null;
      if(classification){
        const noteParts = [];
